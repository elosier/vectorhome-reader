...
function scrapeArticle($, base) {
  ...
  // Per-edition key. Prefer the TITLE: for dated newsletters ("Infolettre du
  // Vendredi 10 juillet") it's stable across re-fetches of the same edition yet
  // unique per edition, so minor post-publish content drift (a relative
  // timestamp, a late edit) updates the article in place instead of forging a
  // duplicate. Fall back to a content hash only when there's no real title
  // (title defaulted to the URL), to still separate untitled editions.
  const hasRealTitle = title && title !== base;
  const editionKey = hasRealTitle ? 't:' + djb2(title.toLowerCase()) : 'c:' + djb2(contentHtml) + '-' + contentHtml.length;
